...
public:
    ...
    IIR_filter(float wz,
               ...
               float Dp,
               float Ts,
               float K); // G(s) = K * wp^2/wz^2 * (s^2 + 2*Dz*wz*s + wz^2)/(s^2 + 2*Dp*wp*s + wp^2)
    IIR_filter(float *b,
               float *a,
               int nb,
               int na); // G(z) = B(z) / A(z) = (b_0 + b_1*z^-1 + ... + b_nb*z^-nb) / (1 + a_1*z^-1 + ... + a_na*z^-na)
    ...
    float operator()(float u) { return filter(u); }
