...
/*
    ...
    init for: - first order differentiatior:     G(s) = s/(T*s + 1)
              ...
              - compl. conj. lead lag with gain: G(s) = K*w0p^2/w0z^2 * (s^2 + 2*Dz*w0z*s + w0z^2)/(s^2 + 2*Dp*w0p*s +
   w0p^2)
              - nth order, with arbitrary values
    - billinear transformation is used for s -> z
    - reseting the filter only makes sense for static signals, whatch out if you're using the differentiator, static
...
// G(s) = s/(T*s + 1)
IIR_filter::IIR_filter(float T, float Ts) { setup(T, Ts); }
...
IIR_filter::IIR_filter(float w0, float D, float Ts, float K) { setup(w0, D, Ts, K); }

// G(s) = K * wp^2/wz^2 * (s^2 + 2*Dz*wz*s + wz^2)/(s^2 + 2*Dp*wp*s + wp^2)
IIR_filter::IIR_filter(float wz, float Dz, float wp, float Dp, float Ts, float K) { setup(wz, Dz, wp, Dp, Ts, K); }

IIR_filter::IIR_filter(float *b, float *a, int nb, int na) { setup(b, a, nb, na); }
...
void IIR_filter::setup(float wz, float Dz, float wp, float Dp, float Ts, float K)
{
    ...
    double Ts_d = static_cast<double>(Ts);
    double K_d = static_cast<double>(K);
    B[0] = static_cast<float>(K_d * (wp_d * wp_d * (Ts_d * Ts_d * wz_d * wz_d + 4.0 * Dz_d * Ts_d * wz_d + 4.0)) /
                              (wz_d * wz_d * (Ts_d * Ts_d * wp_d * wp_d + 4.0 * Dp_d * Ts_d * wp_d + 4.0)));
    B[1] = static_cast<float>(K_d * (2.0 * wp_d * wp_d * (Ts_d * Ts_d * wz_d * wz_d - 4.0)) /
                              (wz_d * wz_d * (Ts_d * Ts_d * wp_d * wp_d + 4.0 * Dp_d * Ts_d * wp_d + 4.0)));
    B[2] = static_cast<float>(K_d * (wp_d * wp_d * (Ts_d * Ts_d * wz_d * wz_d - 4.0 * Dz_d * Ts_d * wz_d + 4.0)) /
                              (wz_d * wz_d * (Ts_d * Ts_d * wp_d * wp_d + 4.0 * Dp_d * Ts_d * wp_d + 4.0)));
    A[0] = static_cast<float>((2.0 * Ts_d * Ts_d * wp_d * wp_d - 8.0) /
                              (Ts_d * Ts_d * wp_d * wp_d + 4.0 * Dp_d * Ts_d * wp_d + 4.0));
    A[1] = static_cast<float>((Ts_d * Ts_d * wp_d * wp_d - 4.0 * Dp_d * Ts_d * wp_d + 4.0) /
                              (Ts_d * Ts_d * wp_d * wp_d + 4.0 * Dp_d * Ts_d * wp_d + 4.0));
...
void IIR_filter::reset() { reset(0.0f, K * 0.0f); }

void IIR_filter::reset(float u) { reset(u, K * u); }
...
void IIR_filter::set_limits(float yMax) { set_limits(-yMax, yMax); }
...
float IIR_filter::get_output() { return yk[0]; }
