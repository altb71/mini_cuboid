...
mpu6500_spi::mpu6500_spi(SPI &_spi, PinName _cs)
    : spi(_spi)
    , cs(_cs)
{
}
...
{
    // unsigned int response; // unused placeholder
    spi.format(8, 0);
    ...
    // Set Gyro oscillator as the clock source. This is done for more accuracy when calculating angles. Datasheet
    // section 4.12 Clocking
    ...
    // Ensure the FCHOICE_B bits in the MPUREG_GYRO_CONFIG register are set to 00 for this to work, specially when
    // configuring the gyro range later on.
    write2spi(MPUREG_CONFIG, 0x03);
    ThisThread::sleep_for(chrono::milliseconds(15));
    // Set Accelerometer Digital Low Pass Filter (DLPF) Bandwidth to 184 Hz (5.80 ms). Used to be at 41 Hz, changed it
    // bc of the delay time. Ensure the ACCEL_FCHOICE_B bit in the MPUREG_ACCEL_CONFIG_2 register are set to 0 for this
    // to work.
...
{
    unsigned int response; // set but not used

    // ENABLE INTERRUPTS
    select();
    response = spi.write(MPUREG_INT_ENABLE);
    response = spi.write(0x01);
    (void)response; // Use response to silence set-but-not-used warning while keeping variable
    deselect();
    return 0;
}
...
void mpu6500_spi::readGyro(void)
{
    uint8_t responseHx, responseLx, responseHy, responseLy, responseHz, responseLz;
    int16_t bit_data;
    // float data; // placeholder not used
    select();
...
{
    uint8_t responseH, responseL, calib_data = 0;
    int temp_scale;
